...
function loadDataApply(data) {
	...
	$(data.rows).each(function(i, v) {
		...
		if(v.deal_person!=null){
		str += "<td>" + v.applicant.employeeInfo.emName + "</td>";
			str += "<td>" + v.deal_person.employeeInfo.emName + "</td>";
		}else{
			str += "<td></td>";
			str += "<td></td>";
		}
		...
		str += "<td>" + v.apDate + "</td>";
		str += "<td>" + v.apDealdate + "</td>";
		str += "<td>" + applyLever(v.apLever) + "</td>";
		str += "<td>" + applyState(v.apState) + "</td>";
		str += "<td>" + v.remark + "</td>";
		str += "<td>";
		str += "<a href="+"javascript:delhisapply('"+v.apId+"\');>删除</a>";
		str += "<span>|</span>";
		str += "<a href="+"javascript:lookword('"+v.accessory+"\');>查看文档</a>";				
		str += "</td>";
		str += "</tr>";
		...
		$(v.list).each(function(ii, vv) {		
			...
			str += " <td>"+vv.goodsItem.goods.gName+"</td>";
			str += " <td>"+vv.goodsItem.goods.fabrics+"</td>";
			str += " <td>"+vv.goodsItem.goods.brandName+"</td>";
			str += " <td>"+vv.goodsItem.color+"</td>";
			str += " <td>"+vv.goodsItem.size+"</td>";
			str += " <td>"+vv.num+"</td>";
			str += "  </tr>";
			}
				});
...
//点击删除
function delhisapply(id) {
    //询问框
    layer.confirm('是否确认删除？', {
        btn: ['确定', '取消'] //按钮
    }, function () {
    	$.post("delDealApply.do", {
			id : id
		}, function(data) {
			if (data.result == 0) {
				layer.msg('删除失败', {
...
function showapplyitem() {
    ...
    $("body").delegate(".btn_applyitem", "click", function () {
